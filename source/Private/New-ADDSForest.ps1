...
function New-ADDSForest {
    ...
    process {
        try {
            ...
            # Read-only — always runs, even under -WhatIf, so the operator sees whether
            # prerequisites are met before deciding whether to proceed.
            ...
            # state (feature/module installs, vault registration, directory creation,
            # forest creation) must live inside this block so -WhatIf reports without acting.
